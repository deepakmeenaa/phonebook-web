...
if ( $mode=="add") //provide the form for making a new contact entry.Pink button on right bottom corner is for ADD CONTACT.
 {
 ...
 if ( $mode=="added") //Data will be added to the database.
 {
 mysqli_query ($conn,"INSERT INTO address (name, phone, email) VALUES ('$name', '$phone', '$email')");
 }

 if ( $mode=="edit") //provide the form for making changes to already existing entry.
 { 
 ...
 if ($mode=="edited") //Data will be edited from the database.
 { 
...
if ($mode=="remove") //Data will be removed from database.
 {
 mysqli_query ($conn,"DELETE FROM address where id=$id");
 Print "<div class='container'>Entry has been removed </div><p>";
 }
 
 //select the data of table address to display the summary of all contacts.
